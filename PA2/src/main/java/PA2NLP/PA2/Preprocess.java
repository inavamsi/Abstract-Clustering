...
public class Preprocess {
	...
	public static void twogram(int n, List<String>[] lol, List<String> merged) throws FileNotFoundException, UnsupportedEncodingException {
		PrintWriter writer = new PrintWriter("2gram.txt", "UTF-8");
		for(int i=0;i<merged.size();i++) {
			for(int j=0;j<merged.size();j++) {
				int no=0;
				for(int a=0;a<lol.length;a++) {
					for(int b=0;b<lol[a].size()-1;b++) {
						if(lol[a].get(b)==merged.get(i) && lol[a].get(b+1)==merged.get(j)) {
							no+=1;
						}
					}
				if(no>n) {
					writer.println(merged.get(i)+" "+merged.get(j));
				}
				}
			}
		}
	...
	public static void threegram(int n, List<String>[] lol, List<String> merged) throws FileNotFoundException, UnsupportedEncodingException {
		PrintWriter writer = new PrintWriter("3gram.txt", "UTF-8");
		for(int i=0;i<merged.size();i++) {
			for(int j=0;j<merged.size();j++) {
				for(int k=0;k<merged.size();k++) {
					int no=0;
					for(int a=0;a<lol.length;a++) {
						for(int b=a+1;b<lol[a].size()-2;b++) {
							
							if(lol[a].get(b)==merged.get(i) && lol[a].get(b+1)==merged.get(j) && lol[a].get(b+2)==merged.get(k)) {
								no+=1;
							}
	...
	public static List<String>[] Start(List<String> data, int size) throws IOException {
		...
		for(int i=0;i<size;i++) {
			...
			String larger = data.get(i);
			String smaller = data.get(i).replaceAll(pattern," ");
			while(! smaller.equals(larger)) {
				larger=smaller;
				smaller=smaller.replaceAll(pattern," ");
			}
			//System.out.println(smaller);
			l.addAll(Tokenize_Lemmatize((smaller)));
			Lem_Tok_List[i] = l;
	...
	public static List<String> Join_NER(Sentence s, List<String> l){   
        ...
        for(i=0;i<nerTags.size()-1;i++) {
        	if(nerTags.get(i).equals("LOCATION") || nerTags.get(i).equals("DURATION") || nerTags.get(i).equals("ORGANIZATION") || nerTags.get(i).equals("DATE") || nerTags.get(i).equals("TIME") || nerTags.get(i).equals("PERCENT") || nerTags.get(i).equals("MONEY")) {
        		continue;
        	}
        	if(nerTags.get(i)==nerTags.get(i+1) && nerTags.get(i)!=nex.get(0)) {
        		String finx=l.get(i);
        		//System.out.println(nerTags.get(i)+"***"+finx);
	...
	public static List<String> removeStopWords(List<String> l){  
        ...
        final String[] specwords= {"say","mr.","ms.","with","without","thus","well","whether","try","want","must","let","want","go","whose","whom"};
        l.removeAll(Arrays.asList(specwords));
        return l;
	}
	
	public static String[] stopWords() throws IOException{
		String prefix ="./data/StopWords.txt";
